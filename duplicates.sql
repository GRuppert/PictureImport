...
SELECT dups.media_file_id, group_concat(dups.place separator  ':') folders, group_concat(dups.date_mod separator  ',') timestamps FROM (
	SELECT mfv.media_file_id, CONCAT(d.description, f.path) place, mfi.date_mod FROM media_file_version mfv LEFT JOIN media_file_instance mfi ON mfi.media_file_version_id = mfv.id LEFT JOIN folder f ON f.id = mfi.folder_id LEFT JOIN drive d ON d.id = f.drive_id WHERE d.id = 3 AND mfv.media_file_id IN (
		SELECT distinct(media_file_version.media_file_id)  FROM media_file_version LEFT JOIN media_file ON  media_file_version.media_file_id = media_file.id WHERE media_file.file_type = 'RAW' GROUP BY media_file_version.media_file_id HAVING count(*) = 2)
        ORDER BY 1, 3
) dups
