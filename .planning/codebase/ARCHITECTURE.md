...
**Version tree:**
- Purpose: Represents successive edits of the same logical image across folders. Each `MediaFileVersion` row has a `parent_id` pointing to the previous version of the same `MediaFile`; the root of a chain is the "original" version (`parent == null`, `isOriginal()` returns `true`).
- Entity: `src/main/java/org/nyusziful/pictureorganizer/DAL/Entity/MediaFileVersion.java` (self-referencing `@ManyToOne parent`).
- View DTO: `src/main/java/org/nyusziful/pictureorganizer/DTO/VersionDTO.java` — a node groups a set of `Folder`s that host the same version bundle; DTO-level parent/children are rebuilt in `MediaGeneralDAOImplHib.loadDirectoryVersions` (`src/main/java/org/nyusziful/pictureorganizer/DAL/DAO/MediaGeneralDAOImplHib.java:119`).
- Rendering: `SummaryController.versionTree` with `VersionTreeCell` (`src/main/java/org/nyusziful/pictureorganizer/UI/Contoller/SummaryController.java:239`).
- Mutation API: `MediaFileVersionService.setAsOriginal`, `setAsInvalid`, `setParent` → `MediaFileVersionDAOImplHib`.
