...
SELECT count(*) FROM pictureorganizer.media_image; -- 375472 375431?
...
SELECT count(*) FROM (SELECT image_id, count(*) FROM pictureorganizer.media_image GROUP BY image_id HAVING count(*) = 1) a; -- 163104
SELECT count(*) FROM (SELECT image_id, count(*) FROM pictureorganizer.media_image GROUP BY image_id HAVING count(*) > 1) a; -- 88882

SELECT count(*) FROM (SELECT mi.image_id, count(*) FROM pictureorganizer.media_image mi LEFT JOIN image i ON i.id = mi.image_id WHERE i.type = 'tif' GROUP BY mi.image_id HAVING count(*) > 1) a; -- 10568
